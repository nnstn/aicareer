...
function emojicountry(s){
    if (s.indexOf("US")>-1) return "<img src='emoji/United-States.png'/> "+s;
    if (s.indexOf("DE")>-1) return "<img src='emoji/Germany.png'/> "+s;
    if (s.indexOf("Japan")>-1) return "<img src='emoji/Japan.png'/> "+s;
    if (s.indexOf("UK")>-1) return "<img src='emoji/United-Kingdom.png'/> "+s;
    if (s.indexOf("SG")>-1) return "<img src='emoji/Singapore.png'/> "+s;
    if (s.indexOf("Singa")>-1) return "<img src='emoji/Singapore.png'/> "+s;
    if (s.indexOf("Hongkong")>-1) return "<img src='emoji/Hong-Kong.png'/> "+s;
    if (s.indexOf("HK")>-1) return "<img src='emoji/Hong-Kong.png'/> "+s;
    if (s.indexOf("KR")>-1) return "<img src='emoji/South-Korea.png'/> "+s;
    if (s.indexOf("TW")>-1) return "<img width='22'  src='emoji/cn.webp'/> "+s;
    if (s.indexOf("CN")>-1) return "<img width='22'  src='emoji/cn.webp'/> "+s;
    if (s.indexOf("IN")>-1) return "<img width='22' src='emoji/in.webp'/> "+s;
    if (s.indexOf("荷兰")>-1) return "<img width='22' src='emoji/ne.webp'/> "+s;
    if (s.indexOf("ams")>-1) return "<img width='22' src='emoji/ne.webp'/> "+s;
    return s;
}
//basic functions
...
function init(){
    ...
    if( localStorage.getItem("username") != null  ){
        ...
        $("[name='check-global']").bootstrapSwitch();
        if( localStorage['globalssl'] ) {
            if(localStorage['globalssl']>0)  $('input[name="check-global"]').bootstrapSwitch('state', true, true);
            else $('input[name="check-global"]').bootstrapSwitch('state', false, false);
        }
    }

    if( getItem("proxy_locked") == 'true' ){
        $("#toomanypasserror").show();
    }else if( getItem("proxy_locked") == true ){
        $("#toomanypasserror").show();
    }
    else{
        $("#toomanypasserror").hide();
    }

    if( $('input[name="check-global"]') ){

        $('input[name="check-global"]').on('switchChange.bootstrapSwitch', function(event, state) {
            if(localStorage['globalssl']>0) {
                localStorage['globalssl'] =0;
                $('input[name="check-global"]').bootstrapSwitch('state', false, false);

            }
            else {
                localStorage['globalssl'] =1;
                $('input[name="check-global"]').bootstrapSwitch('state', true, true);
...
if( $("#username").length ){
    ...
    document.getElementById('username').addEventListener('keyup', function() {
        if( $("#username").val()!= localStorage['connected']){
            changeBtStatus(1);
            saveChanges("username");
            removeProxy();
        }
...
if( $("#password").length ){
    document.getElementById('password').addEventListener('blur', function() {
        if( $("#password").val()!= localStorage['password']){
            changeBtStatus(1);
            removeProxy();
        }
    });
...
function saveBlackDomainList(){
    ...
    var url="https://678in.com/index.php?action=saveBlackDomainList&username="+localStorage['username']+"&password="+localStorage['password']+"&domains="+domains;
    $.getScript(url);

}
function reloadServers(){
    $.getScript("https://678in.com/index.php?action=serverlistjson&callback=loadServerlist");
}
function loadServerlist(result) {
    ...
    if(   typeof(result)!="string" ){
        $.each(result, function(val, text) {
            ...
            if(  serverlisthtml.indexOf(val) == -1) {
                $('#serverlist').append($('<tr rel="'+val+'"><td>'+emojicountry(text)+'<small></small></td> </tr>'));
    ...
    $("#serverlist  tr ").bind("click",function(){
        ...
        localStorage['httpsserver'] = $(this).attr("rel");
        saveChanges("serverdomain");
        isReconnect=1;
        if(localStorage["proxy_locked"])localStorage["proxy_locked"] =false;
        setproxy();
    });
}
...
function connect(){
    ...
    var userpass =  CryptoJS.MD5($('#username').val()+""+$('#password').val());
    if( localStorage['userpasscache']  ){
        if( localStorage['userpasscache'] == userpass ){
            // already logged once
            changeBtStatus(0);
    ...
    url = ("https://678in.com/sslspeedy.php?login=1&callback=setproxy&user="+$('#username').val()+"&pass="+$('#password').val());
    $.getScript(url);
    reloadServers();
...
function setproxy(result){
    ...
    if(isReconnect==0){
        ...
        //
        var userpasscache =  CryptoJS.MD5( localStorage['username'] +""+ localStorage['password'] );
        localStorage['userpasscache'] = userpasscache;
...
function refreshconnection(){ isReconnect=1;  setproxy()  ;  }
